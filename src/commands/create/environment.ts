  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (args.config_path) {
        ...
        environmentGraph = await environment.getGraph(args.name, this.componentStore);
      }

      const targetGraph = await datacenterRecord.config.enrichGraph(environmentGraph, args.name);
