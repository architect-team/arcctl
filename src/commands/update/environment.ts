...
export class UpdateEnvironmentCmd extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    const targetDatacenterName = flags.datacenter || environmentRecord?.datacenter;
    const targetDatacenter = targetDatacenterName ? await this.datacenterStore.get(targetDatacenterName) : undefined;
    if (!targetDatacenter) {
    ...
    if (args.config_path) {
      ...
      targetGraph = await targetEnvironment.getGraph(args.name, this.componentStore);
    }

    targetGraph = await targetDatacenter.config.enrichGraph(targetGraph, args.name);

    let startingPipeline = new Pipeline();
    if (environmentRecord?.datacenter) {
      const startingDatacenter = await this.datacenterStore.get(environmentRecord.datacenter);
      if (startingDatacenter) {
        startingPipeline = await this.getPipelineForDatacenter(startingDatacenter);
      }
    }
    ...
    return pipeline
      ...
      .then(async () => {
        await this.saveDatacenter(targetDatacenter.name, targetDatacenter.config, pipeline);
