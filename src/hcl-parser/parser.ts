...
const astToCode = (ast: Program | Node): any => {
  ...
  switch (ast.type) {
    case 'Program': {
      return astToCode(ast.body[0]);
    }
    case 'ExpressionStatement': {
      return astToCode(ast.expression);
    }
...
/**
 * Find and replace ${...} expressions found in the input string
 */
export const replaceExpressions = (str: string, replacer: (matcher: string, key: string) => string): string => {
  let result = str;

  let currentIndex = 0;
  while (true) {
    const expressionStart = result.indexOf('${', currentIndex);
    if (expressionStart === -1) {
      break;
    } else if (result.indexOf('{', expressionStart + 2) === expressionStart + 2) {
      // Escape hatch for application expressions (e.g. ${{ ... }})
      break;
    }

    let braceCount = 1;
    for (let i = expressionStart + 2; i < result.length; i++) {
      if (result[i] === '{') {
        braceCount++;
      } else if (result[i] === '}') {
        braceCount--;
      ...
      if (braceCount === 0) {
        const match = result.substring(expressionStart, i + 1);
        const key = result.substring(expressionStart + 2, i);
        const value = replacer(match, key);
        result = result.substring(0, expressionStart) + value + result.substring(i + 1);
        currentIndex = expressionStart + value.length;
        break;
      }
    }
  }
...
export const applyContext = (
  ...
  while (previous !== JSON.stringify(obj)) {
    ...
    Object.entries(obj).forEach(([key, value]) => {
      ...
      obj[key] = replaceExpressions(value.toString(), (_: string, expression_str: string) => {
        ...
        const res = astToCode(ast as any);
