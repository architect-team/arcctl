...
import { AppNode } from './node.ts';

export type AppGraphOptions = {
  nodes?: AppNode[];
  edges?: AppEdge[];
};
...
export class AppGraph {
  ...
  constructor(options?: AppGraphOptions) {
    this.nodes = options?.nodes || [];
    this.edges = options?.edges || [];
  }

  public insertNodes(...args: AppNode[]): AppGraph {
  ...
  public insertEdges(...args: AppEdge[]): AppGraph {
  ...
  public removeNode(id: string): AppGraph {
  ...
  public removeEdge(options: { from?: string; to?: string }): AppGraph {
  ...
  public validate(): void {
    for (const node of this.nodes) {
      ...
      if (node.type === 'secret') {
        const secret_node = node as AppNode<'secret'>;
